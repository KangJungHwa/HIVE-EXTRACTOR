...
import com.dd.api.model.Condition;
...
public class QueryGenerator {
     ...
     * @return
     */
    public String genFullQuery(ExtractCondition extractCondition){
        String WHERE_DUMMY_TEMPLATE=" WHERE 1=1 ";
        String fullQuery=
                getSelectCause(extractCondition.getSelectColumn()) +
                        getFromCause(extractCondition.getFromTable()) +
                        WHERE_DUMMY_TEMPLATE +
                        getWhereFromCause(extractCondition.getWhereFrom()) +
                        getWhereToCause(extractCondition.getWhereTo());
                if(extractCondition.getWhereIn()!=null) {
                    fullQuery=fullQuery + getWhereInCause(extractCondition.getWhereIn());
                }
                if(extractCondition.getWhereEqual() != null) {
                    fullQuery=fullQuery + getWhereEqualCause(extractCondition.getWhereEqual());
                }
